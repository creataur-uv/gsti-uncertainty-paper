...
def cross_val_scores(
    ...
    while i < num_Folds:
        ...
        model = dGP.DeepGPR(train_X.shape[-1],
                            hl1_out_dims=16,
                            hl2_out_dims=8,
                            train_x=train_X, train_y=train_y, batch_size=1024)
        model.cuda()
        model.double()

        start = time.time_ns()
        model.optimize(num_epochs=200, num_samples=10, num_obs=train_X.shape[-2])
        ...
        result = helpers.benchmark.benchmark(observed.numpy(), predicted.numpy())
        print(result['R2'])
...
# for trait=leaf_traits[0]:
    ...
    for num_PC in range(1, 50):
        ...
        to_DB["trait"] = trait
        to_DB["mlra"] = "CV_PCA_DeepGPR_16_8"
        to_DB["num_PC"] = num_PC

        # # Data to Data Base
        print("Saving to Data Base")
        DB_Name = f"data/output/CV_PCA_DeepGPR_16_8.db"
