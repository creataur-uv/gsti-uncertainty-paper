...
def cross_val_scores(
    ...
    while i < num_Folds:
        ...
        kernel = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(nu=1.5, ard_num_dims=train_X.size(-1)))
...
# for trait=leaf_traits[0]:
    ...
    for num_PC in range(1, 50):
        ...
        to_DB["trait"] = trait
        to_DB["mlra"] = "CV_PCA_GPR_MATERN_3_2"

        # Data to Data Base
        print("Saving to Data Base")
        DB_Name = f"data/output/CV_PCA_GPR_MATERN_3_2.db"
        table_Name = "simulations"

        myDB = db.db(DB_Name, table_Name)

        myDB.insert(f"""{to_DB["mlra"]}_{trait}_{to_DB['num_PC']}""", to_DB)
        myDB.closeDB()
